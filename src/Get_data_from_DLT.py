...
#         self.excel.update(utterance=self.utterance)
#         check_cpu = False
...
#                     check_cpu = True
#                     print("final asr true")
#                     self.Log_analyzer(line)
#                     # break
#                 if 'INTENT=' in line and 'CONFIDENCE='in line:
#                     print("INTENT In line")
#                     print(line)
...
#         self.excel.update(recognized_text=utterance,intent=intent,confidence=confidence,intent_domain=intent_domain)
...
class Get_data():
    ...
    def Log_analyzer(self,logline):
        ...
        if match:
            ...
            # print("No orthography field found.")
    def Intent_analyzer(self,log_line):
        print("called intent analyzer")
        intent_match = re.search(r'INTENT=([^,]+)', log_line)
        intent = intent_match.group(1).strip() if intent_match else "No Intent"
        # INTENT_DOMAIN
        domain_match = re.search(r'INTENT_DOMAIN=([^,]+)', log_line)
        intent_domain = domain_match.group(1).strip() if domain_match else "No domain"
        # CONFIDENCE
        confidence_match = re.search(r'CONFIDENCE=([^,]+)', log_line)
        confidence = confidence_match.group(1).strip() if confidence_match else "Null"
        # SYSTEM_LANGUAGE
        lang_match = re.search(r'SYSTEM_LANGUAGE=([^,]+)', log_line)
        language = lang_match.group(1).strip() if lang_match else None
        # UTTERANCE
        utt_match = re.search(r'UTTERANCE=([^,]+)', log_line)
        utterance = utt_match.group(1).strip() if utt_match else "No utterance match"
        # Print the values
        # print("Intent:", intent)
        # print("Domain:", intent_domain)
        # print("Confidence:", confidence)
